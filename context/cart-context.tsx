...
export function CartProvider({ children }: { children: React.ReactNode }) {
  ...
  const fetchShopifyCart = useCallback(async (cartId: string) => {
    dispatch({ type: "SET_LOADING", payload: true })
    ...
    try {
      ...
      // Clear local cart if cart cannot be loaded
  ...
  const addItem = useCallback(
    ...
    [state.shopifyCartId, fetchShopifyCart, createFreshCart],
  )
  ...
  const removeItem = useCallback(
    ...
    [state.shopifyCartId, fetchShopifyCart],
  )
  ...
  }, [state.shopifyCartId, state.items, fetchShopifyCart])
